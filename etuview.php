...
<body>
    ...
    <img src="imgs/brand.png" class="rounded-circle" alt="brand" style="width :50px;height:50px;">
        ...
        <div class="row bg-warning px-4 pt-3" style='padding-bottom : 80px;'>
            <div class="col-md">
                <div> <span class="d-inline-block mr-3" style="border-bottom : 3px solid rgba(0,0,0,0.3);">Profil</span> <small class='bg-info text-white py-1 px-4 rounded-pill'><?= $user_data['3'] ?></small> <br> <span class="text-dark font-weight-bold d-inline-block ml-1 mt-2" >  <?= $user_data['1'] .' '.$user_data['0']?> <br> <small class='text-dark font-weight-bold py-1 px-4 rounded-pill'><?= $_SESSION['role'] ?> </small>
            </div>
        </div>
         ...
         <?php $filiere_content_req = $pdo -> query('SELECT * FROM users WHERE purpose ='.$filiere['id'].'&& active_user = \'1\'') ; ?>
             <ul class="list-group list-group-flush">
                 ...
                 <?php foreach($filiere_content_req->fetchAll(PDO::FETCH_ASSOC) as $filiere_content): ?>
                     ...
                     ?>
                     <a href="etu_info.php?etuid=<?php echo $filiere_content['id'];  ?>" class="list-group-item-action px-0 py-2">
