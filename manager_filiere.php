...
<body>
    ...
    <div class="col-md-4 list-group">
        <?php foreach($filieres as $key => $filiere) : ?>
            ...
            <a class="list-group-item list-group-item-action<?= $activeClass ?>" id="pills-filieres-<?= $filiere['id'] ?>" data-toggle="pill" href="#pills_filiere_<?=$filiere['id']?>" role="tab" aria-controls="pills-home" aria-selected="true" ><?= $filiere['nom'] ?><span class="badge badge-pill badge-secondary d-inline-block float-right mr-2">
                <?= $filiere['Netudiants'] ?>
            </span></a>
        <?php endforeach ?>
    </div>
    ...
    <?php foreach($filieres as $key => $filiere): ?>
        ...
        <div class="tab-pane fade show<?=$activeClass?> col-md-12" role="tabpanel" aria-labelledby="pills-profile-tab" id="pills_filiere_<?=$filiere['id']?>">
            <form action="filiere_management.php?fid=<?= $filiere['id']?>&flmg=upd" method="POST">
                <div class="form-group">
                    <input type="text" name="filiere_name" class="form-control w-75 mx-auto" value="<?= $filiere['nom']?>" >
                    <button type="submit" class="btn btn-info w-25 d-block mx-auto my-3">Modifier</button>
                </div>
            </form>  
            <div class="d-flex justify-content-between mt-5">
                <a href="filiere_management.php?fid=<?= $filiere['id']?>&flmg=actf" class="btn btn-sm btn-secondary rounded-pillpill<?= ($filiere['filiere_statut'] == 1) ? ' disabled' :' active' ?>">Activer filiere</a>
                <a href="filiere_management.php?fid=<?= $filiere['id']?>&flmg=del" class="btn btn-sm btn-secondary rounded-pillpill<?= ($filiere['filiere_statut'] == -1) ? ' disabled' :' active' ?>">supprimer filiere</a>
                <a href="filiere_management.php?fid=<?= $filiere['id']?>&flmg=disbl" class="btn btn-sm btn-secondary rounded-pillpill<?= ($filiere['filiere_statut'] == 0) ? ' disabled' :' active' ?>">Desactiver filiere</a>
            </div>
        </div>    
